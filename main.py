...
WEATHER_ENABLED = True  # Global toggle for weather effects
...
while running:
    ...
    for event in events:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_7: debug_mode = not debug_mode; print(f"Debug Mode {'ACT' if debug_mode else 'DEACT'}IVATED")
                ...
                # Toggle weather effects on/off with the 1 key
                WEATHER_ENABLED = not WEATHER_ENABLED
                if WEATHER_ENABLED:
                    # If weather was re-enabled, recreate the particles
                    weather_particles.clear()
                    for _ in range(WEATHER_PARTICLE_COUNT.get(current_weather, 0)):
                        weather_particles.append(WeatherParticle(current_weather, SCREEN_WIDTH, SCREEN_HEIGHT))
                    print("Weather effects enabled")
                    # Display weather enabled message
                    weather_message_text = "Weather effects enabled"
                    weather_message_timer = WEATHER_MESSAGE_DURATION
                else:
                    # If weather was disabled, clear all particles
                    weather_particles.clear()
                    print("Weather effects disabled")
                    # Display weather disabled message
                    weather_message_text = "Weather effects disabled"
                    weather_message_timer = WEATHER_MESSAGE_DURATION
    ...
    if current_game_state == "WELCOME":
        ...
        if match_active:
            ...
            # --- Weather Updates ---
            if weather_particles_enabled and WEATHER_ENABLED:
            ...
            if current_weather == "WINDY" and WEATHER_ENABLED:
    ...
    # Override if weather has a specific background color and weather is enabled
    if WEATHER_ENABLED and "background_color" in weather_effect:
    ...
    # Draw weather effects in the background only if enabled
    if WEATHER_ENABLED:
        for p in weather_particles:
            p.draw(screen)
        
    # Add fog overlay if foggy and weather is enabled
    ...
    if current_weather == "FOGGY" and WEATHER_ENABLED:
        fog_surf = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
    ...
    # Draw Weather Toggle Status (Always visible)
    weather_toggle_font = font_small
    weather_toggle_text = f"Weather: {'ON' if WEATHER_ENABLED else 'OFF'}"
    weather_toggle_color = (0, 200, 0) if WEATHER_ENABLED else (200, 0, 0)  # Green for ON, Red for OFF
    
    # Render toggle text
    weather_toggle_surf = weather_toggle_font.render(weather_toggle_text, True, weather_toggle_color)
    weather_toggle_rect = weather_toggle_surf.get_rect(top=40, right=SCREEN_WIDTH - 10)
    
    # Add background for text
    toggle_bg = weather_toggle_rect.inflate(10, 6)
    toggle_bg_surf = pygame.Surface(toggle_bg.size, pygame.SRCALPHA)
    toggle_bg_surf.fill((0, 0, 0, 150))
    
    # Draw to screen
    screen.blit(toggle_bg_surf, toggle_bg.topleft)
    screen.blit(weather_toggle_surf, weather_toggle_rect)
